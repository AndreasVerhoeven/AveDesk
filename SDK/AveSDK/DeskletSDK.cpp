...
{
	char szNumber[100] = {0};
	//_itoa_s(iValue, szNumber, sizeof(szNumber)/sizeof(szNumber[0]), 10);
...
int ShowContextMenu(HWND owner, HMENU menu, POINT* p, const char* filename)
{
	...
	//strcpy_s(file,MAX_PATH, filename);
	...
	//strcpy_s(filepath, MAX_PATH, file);
	...
	for(int i=strlen(file)-1;i>0;--i)
	{
		if(file[i]=='\\' || file[i]=='/')
		{
			...
			//strcpy_s(file,MAX_PATH,file+strlen(filepath)+1);
			break;
		}
	}
...
//-----------------------------------------------------------------------------
typedef unsigned int (CALLBACK *HDeskletLayerAddExDUMMY)(HWND, const char*, const char*, DWORD);
HDeskletLayerAddExDUMMY HDeskletLayerAddEx = (HDeskletLayerAddExDUMMY) GetProcAddress(GetModuleHandle(0), "DeskletLayerAddEx");
unsigned int CALLBACK DeskletLayerAddEx(HWND hwndDesklet, const char* name, const char* nameOfParent, DWORD flags)
{
	if(!HDeskletLayerAddEx)
		return (unsigned int)-1;

	return HDeskletLayerAddEx(hwndDesklet, name, nameOfParent, flags);
}
//-----------------------------------------------------------------------------
typedef BOOL (CALLBACK *HDeskletLayerGetImageClipRectDUMMY)(HWND, unsigned int, BOOL*, RECT*,int*, int*);
HDeskletLayerGetImageClipRectDUMMY HDeskletLayerGetImageClipRect = (HDeskletLayerGetImageClipRectDUMMY) GetProcAddress(GetModuleHandle(0), "DeskletLayerGetImageClipRect");
BOOL CALLBACK DeskletLayerGetImageClipRect(HWND hwndDesklet, unsigned int id, BOOL* useClipping, RECT* clipRect,
										int* xScale, int* yScale)
{
	if(!HDeskletLayerGetImageClipRect)
		return FALSE;

	return HDeskletLayerGetImageClipRect(hwndDesklet, id, useClipping, clipRect, xScale, yScale);
}
//-----------------------------------------------------------------------------
typedef BOOL (CALLBACK *HDeskletLayerGetPlacementDUMMY)(HWND, unsigned int, BOOL*, RECT*,int*, int*);
HDeskletLayerGetPlacementDUMMY HDeskletLayerGetPlacement = (HDeskletLayerGetPlacementDUMMY) GetProcAddress(GetModuleHandle(0), "DeskletLayerGetPlacement");
BOOL CALLBACK DeskletLayerGetPlacement(HWND hwndDesklet, unsigned int id, BOOL* usePlacement, RECT* placementRect, 
									int* xScale, int* yScale)
{
	if(!HDeskletLayerGetPlacement)
		return FALSE;

	return HDeskletLayerGetPlacement(hwndDesklet, id, usePlacement, placementRect, xScale, yScale);
}
//-----------------------------------------------------------------------------
typedef BOOL (CALLBACK *HDeskletLayerGetIsNotMovableDUMMY)(HWND, unsigned int);
HDeskletLayerGetIsNotMovableDUMMY HDeskletLayerGetIsNotMovable = (HDeskletLayerGetIsNotMovableDUMMY) GetProcAddress(GetModuleHandle(0), "DeskletLayerGetIsNotMovableDUMMY");
BOOL CALLBACK DeskletLayerGetIsNotMovable(HWND hwndDesklet, unsigned int id)
{
	if(!HDeskletLayerGetIsNotMovable)
		return FALSE;

	return HDeskletLayerGetIsNotMovable(hwndDesklet, id);
}
//-----------------------------------------------------------------------------
typedef BOOL (CALLBACK *HDeskletLayerGetIsClickThroughDUMMY)(HWND, unsigned int);
HDeskletLayerGetIsClickThroughDUMMY HDeskletLayerGetIsClickThrough = (HDeskletLayerGetIsClickThroughDUMMY) GetProcAddress(GetModuleHandle(0), "DeskletLayerGetIsClickThrough");
BOOL CALLBACK DeskletLayerGetIsClickThrough(HWND hwndDesklet, unsigned int id)
{
	if(!HDeskletLayerGetIsClickThrough)
		return FALSE;

	return HDeskletLayerGetIsClickThrough(hwndDesklet, id);
}
//-----------------------------------------------------------------------------
...
typedef BOOL (CALLBACK *HDeskletLayerSetToolTipTextDUMMY)(HWND, unsigned int, const WCHAR*);
HDeskletLayerSetToolTipTextDUMMY HDeskletLayerSetToolTipText = (HDeskletLayerSetToolTipTextDUMMY)GetProcAddress(GetModuleHandle(0), "DeskletLayerSetToolTipText");
BOOL CALLBACK DeskletLayerSetToolTipText(HWND hwndDesklet, unsigned int id, const WCHAR* text)
{
	if(!HDeskletLayerSetToolTipText)
		return FALSE;

	return HDeskletLayerSetToolTipText(hwndDesklet, id, text);
}
