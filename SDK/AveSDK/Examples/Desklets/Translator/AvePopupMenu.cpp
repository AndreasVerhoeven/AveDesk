...
#define AVE_Ave_MENU_FONT           TEXT("Lucida Sans Unicode")
#define AVE_Ave_MENU_STYLES         FontStyleBold
#define AVE_Ave_MENU_FONTSIZE       12
#define AVE_Ave_MENU_FONTCOLOR      Color(255, 0, 0, 0)
#define AVE_Ave_MENU_SELFONTCOLOR   Color(255, 255, 255, 255)
#define AVE_Ave_BIGGEST_MENU_WIDTH 1024
...
UINT GetStringLength(const WCHAR* str, UINT itemHeight)
{
	// 1024
	Rect rect(0,0,AVE_Ave_BIGGEST_MENU_WIDTH, itemHeight);
	
	OLECHAR wtext[10024] = {0};
#ifndef UNICODE
	MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, AVE_Ave_MENU_FONT , -1, wtext, sizeof(wtext));
#else
	wcscpy(wtext, AVE_Ave_MENU_FONT);
#endif
	...
	path.AddString(str, -1, &fontFamily, AVE_Ave_MENU_STYLES, (float)AVE_Ave_MENU_FONTSIZE, rect, &format);
...
void DrawItem(const WCHAR* str, Graphics& g, Rect& r, MenuResources* res, BOOL selected, BOOL mouseOver)
{
	if(NULL == res)
		return;
	...
	if(str != NULL)
	{
		...
		Gdiplus::FontFamily  fontFamily(wtext);
		SolidBrush  black(mouseOver ? AVE_Ave_MENU_SELFONTCOLOR : AVE_Ave_MENU_FONTCOLOR);
...
BOOL CopyMenuData(InternalMenuData* internal, const MenuData* data)
{
	...
	internal->data.items = new WCHAR*[internal->data.count];
	...
	for(UINT i = 0; i < internal->data .count; ++i)
	{
		if(data->items[i] != NULL)
			internal->data .items[i] = wcsdup(data->items[i]);
		else
			internal->data.items = NULL;
	}
...
#define AVE_Ave_MENU_CLASS TEXT("AveAveMenuClass")
...
ATOM RegisterMenuClass()
{
	...
	cls.lpszClassName = AVE_Ave_MENU_CLASS;
...
BOOL InitializeMenu(HWND parent, UINT notifyMessage, const MenuData* items, const char* resourcesPath, const POINT* pt)
{
	...
	HWND hwnd = CreateWindowEx(exStyle, AVE_Ave_MENU_CLASS, TEXT(""), style, 0, 0, s.cx, s.cy, NULL, NULL, HINST_THISCOMPONENT, reinterpret_cast<LPVOID>(data));
	if(NULL == hwnd)
	{
...
#define AVE_Ave_INTERNAL_MENU_DATA_PROP_NAME TEXT("menu_data")

#define AVE_Ave_FADEIN_TIMER_ID  2949
#define AVE_Ave_FADEOUT_TIMER_ID 2950
#define AVE_Ave_ALPHA_DELTA      20
#define AVE_Ave_ALPHA_MAX        240
#define AVE_Ave_TIMER_INTERVAL   10
...
LRESULT OnMenuCreate(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	BOOL bRes = SetProp(hwnd, AVE_Ave_INTERNAL_MENU_DATA_PROP_NAME, reinterpret_cast<void*>(data) );
	if(!bRes)
		return -1;
	...
	data->alpha = AVE_Ave_ALPHA_DELTA;
	SetBitmapToWindow(hwnd, bmp, data->pt, data->alpha);
	delete bmp;

	::SetTimer(hwnd, AVE_Ave_FADEIN_TIMER_ID, AVE_Ave_TIMER_INTERVAL, NULL);
...
{
	::KillTimer(hwnd, AVE_Ave_FADEIN_TIMER_ID);
	::SetTimer(hwnd, AVE_Ave_FADEOUT_TIMER_ID, AVE_Ave_TIMER_INTERVAL, NULL);
	return 0;
}

LRESULT OnMenuTimer(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	if(AVE_Ave_FADEIN_TIMER_ID == wParam)
	{
		InternalMenuData* data = reinterpret_cast<InternalMenuData*>(GetProp(hwnd, AVE_Ave_INTERNAL_MENU_DATA_PROP_NAME));
		if(data != NULL)
		{
			if(AVE_Ave_ALPHA_MAX - AVE_Ave_ALPHA_DELTA > data->alpha)
			{
				data->alpha += AVE_Ave_ALPHA_DELTA;
	...
	if(AVE_Ave_FADEOUT_TIMER_ID == wParam)
	{
		InternalMenuData* data = reinterpret_cast<InternalMenuData*>(GetProp(hwnd, AVE_Ave_INTERNAL_MENU_DATA_PROP_NAME));
		if(data != NULL)
		{
			if(AVE_Ave_ALPHA_DELTA < data->alpha)
			{
				data->alpha -= AVE_Ave_ALPHA_DELTA;
...
{
	// FIX: fixed bug with WM_DESTROY being called twice and thus free the menu data twice.
	//      When there was a focus change in the OnSelChange handler of a combobox,
	//      the WM_KILLFOCUS handler would also do a DestroyWindow.
	//      Fixed it by removing the window-property that holds the data
	//		on the first WM_DESTROY.
	InternalMenuData* data = reinterpret_cast<InternalMenuData*>(RemoveProp(hwnd, AVE_Ave_INTERNAL_MENU_DATA_PROP_NAME));
	if(data != NULL)
	{
...
LRESULT OnMenuLButtonUp(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(data != NULL)
	{
		if(AVE_Ave_ALPHA_MAX == data->alpha) // means: animation is done
		{
			int topCorrection = data->resources.top_mid->GetHeight();
			int itemHeight = data->resources.mid_mid->GetHeight();
			int leftCorrection = data->resources.mid_left->GetWidth();
