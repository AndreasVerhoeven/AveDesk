...
{	\
	strcpy_s(szName,128, a);	\
	strcpy_s(szAuthor, 128, b); \
	strcpy_s(szNotes, 1024, c); \
	*piVersion = d; \
}

#define REQUIRED_EVENT_ONCREATE(T) T* __stdcall OnCreate(HWND hwnd, HINSTANCE hInstance,const char* ini, const char* group) \
{ \
...
#define REQUIRED_EVENT_ONDESTROY(T) void __stdcall OnDestroy(T* t, HWND hWnd)\
{\
	delete t;\
	t = NULL;\
};
...
#define EVENT_ONLEFTBUTTONCLICK(T) BOOL __stdcall OnLeftButtonClick(T* t ,POINT *ptCursor, SIZE *sizeDesklet)\
{\
...
#define EVENT_ONRIGHTBUTTONCLICK(T) BOOL __stdcall OnRightButtonClick(T* t ,POINT *ptCursor, SIZE *sizeDesklet)\
{\
...
#define EVENT_ONMOUSEMOVE(T) BOOL __stdcall OnMouseMove(T* t, int x, int y)\
{\
...
#define EVENT_ONMOUSEON(T)  BOOL __stdcall OnMouseOn(T* t)\
{\
	return t->MouseOn();\
}
...
#define EVENT_ONMOUSEOUT(T) BOOL __stdcall OnMouseOut(T* t)\
{\
	return t->MouseOut();\
}
...
#define EVENT_ONDRAGENTER(T) BOOL __stdcall OnDragEnter(T* t,IDataObject* obj,DWORD keys ,POINTL pnt ,DWORD* effect)\
{\
...
#define EVENT_ONDRAGLEAVE(T) BOOL __stdcall OnDragLeave(T* t)\
{\
	return t->DragLeave();\
}
...
#define EVENT_ONDRAGOVER(T) BOOL __stdcall OnDragOver(T* t,DWORD keys ,POINTL pnt ,DWORD* effect)\
{\
...
#define EVENT_ONDROP(T) BOOL __stdcall OnDrop(T* t,IDataObject* obj,DWORD keys ,POINTL pnt ,DWORD* effect)\
{\
...
#define EVENT_ONSAVE(T)  void __stdcall OnSave(T* t, const char *szIni, const char *szIniGroup, BOOL bIsForExport)\
{\
...
#define EVENT_ONPROCESSMESSAGE(T) void __stdcall OnProcessMessage(T *t,HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)\
{\
...
#define EVENT_ONADDPROPERTYSHEETS(T) int __stdcall OnAddPropertySheets(T* t, int numPresent)\
{\
...
#define EVENT_ONDRAGDROPDATAREQUEST(T) BOOL __stdcall OnDragDropDataRequest(T* t,FORMATETC* pFetc, STGMEDIUM* pMedium)\
{\
...
#define EVENT_ONSELECT(T) BOOL __stdcall OnSelect(T* t)\
{\
	return t->Select();\
}
...
#define EVENT_ONDESELECT(T)  BOOL __stdcall OnDeselect(T* t)\
{\
	return t->Deselect();\
}
...
#define EVENT_ONMOUSEWHEELFORWARD(T) BOOL __stdcall OnMouseWheelForward(T* t, DWORD keysDown, int numScrolls, int x, int y)\
{\
...
#define EVENT_ONMOUSEWHEELBACKWARD(T) BOOL __stdcall OnMouseWheelBackward(T* t, DWORD keysDown, int numScrolls, int x, int y)\
{\
