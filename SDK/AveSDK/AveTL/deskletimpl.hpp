...
#define PROCESSMESSAGE_BEGIN() void ProcessMessage(HWND hwnd,UINT msg, WPARAM w,LPARAM l){
#define HANDLE_MESSAGE(t,f) if(msg == t ) f(msg,w,l);
#define PROCESSMESSAGE_END() }
...
public:
	...
	BOOL Flip(const FlipParams& params)
	{
		assert( m_hWnd != NULL);

		return ::DeskletFlip(m_hWnd, &params);
	}

	BOOL SetClosePoint(const POINT& p, int xScale, int yScale)
	{
		assert( m_hWnd != NULL);
		return ::DeskletSetClosePoint(m_hWnd, &p, xScale, yScale);
	}

	BOOL SetNoClosePoint()
	{
		assert( m_hWnd != NULL);

		return ::DeskletSetClosePoint(m_hWnd, NULL, 0, 0);
	}

	void SetImageCanBeSet(BOOL imageCanBeSet)
	{
		assert( m_hWnd != NULL);

		::DeskletSetImageSetStatus(m_hWnd, imageCanBeSet);
	}

	void FadeIn(BOOL async=FALSE, DWORD duration = USE_DEFAULT_DURATION)
	{
		assert( m_hWnd != NULL);

		::DeskletFadeIn(m_hWnd, async, duration);
	}

	void FadeOut(BOOL async=FALSE, DWORD duration = USE_DEFAULT_DURATION)
	{
		assert( m_hWnd != NULL);

		::DeskletFadeOut(m_hWnd, async, duration);	
	}

	BOOL AsyncSetPointer()
	{
		assert( m_hWnd != NULL);

		T* t = static_cast<T*>(this);

		return ::DeskletAsyncSetPointer(m_hWnd,reinterpret_cast<void*>(t));
	}
	...
	std::string GetDLLDirectory()const
	{
		...
		::GetDLLPath(m_hInstance,buf,MAX_PATH);
	...
	std::string GetDLLFilePath()const
	{
		...
		::GetModuleFileName(m_hInstance,buf,MAX_PATH);
	...
	BOOL SaveW(const std::wstring& path)
	{
		assert( m_hWnd != NULL);

		return ::DeskletSaveW(m_hWnd,path.c_str() );
	}
	...
	void Move(int left, int top, int width, int height, BOOL bRedraw=TRUE)
	{
		assert( m_hWnd != NULL);
