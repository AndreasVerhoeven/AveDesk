...
/**
 * Called when a the host application needs information about a cache entry for the skins preview
 * bitmaps. The callee should fill in the cacheEntry structure with a filename and timestamp.
 *
 * @param lpEnumData - the pointer returned by OnCreateSkinEnumeratorW() for which information about a skin
 *						is requested.
 * @param skin - a pointer to a SkinInfo structure describing the skin that cache information is requested
 *				 about.
 * @param cacheData - a pointer to the cache entry to be filled by the callee. cacheData->type will have
 *						CACHE_TYPE_LARGE_BITMAP if the entry data for the large bitmap is 
 *						requested and CACHE_TYPE_SMALL_BITMAP for the small, iconic bitmap.
 *
 * @return AVEDESK_CACHE_ENTRY_FILLED if cacheData is filled out, AVEDESK_CACHE_NOT_SUPPORTED if
 *			bitmap caching is not supported.
 *
 * @remark this function can be called from a different different thread that the thread
 *			that created the lpEnumData pointer.
 *
 */
UINT CALLBACK OnGetSkinPreviewBitmapCacheEntry(ENUMERATOR_DATA* lpEnumData, const SkinInfo* skin, 
				CacheEntry* cacheData);


/**
 * Called when AveDesk requests a bitmap preview for a skin. The function should block
 * until either the bitmap creation is finished or until requestData->bStop is TRUE, in 
 * which case the bitmap doesn't have to be created anymore.
 * When AveDesk wants the routine to stop creating the bitmap, it will set the 
 * requestData->bStop to TRUE and call OnStopSkinPreviewBitmap() with the same requestData
 * to allow the desklet to use other ways of stopping the creation of the bitmap. To identify
 * the request, the desklet can set the requestData->lParam field to something that identifies
 * the request-handler (e.g. a handle to a thread or a pointer to an object dealing with the 
 * bitmap generation).
 * 
 * @param lpEnumData - the pointer returned by OnCreateSkinEnumeratorW() for which information about a skin
 *						is requested.
 * @param skin - a pointer to a SkinInfo structure describing the skin that cache information is requested
 *				 about.
 * @param requestData - a pointer to the requestData that is used for information setting [in], and
 *						information retrieval [out]. See the structure for more details.
 *						requestData->flags will have PREVIEW_REQUEST_WANT_LARGE_BITMAP set for the large,
 *						bitmap and PREVIEW_REQUEST_SMALL_BITMAL set for the small, iconic bitmap.
 *						If both small and large bitmaps are the same actually, the
 *						callee can set PREVIEW_REQUEST_SMALL_AND_BIG_SAME to the flags to indicate this.
 *						The callee should make the small bitmap the size fo requestData->size,
 *						but for the big bitmap, the callee should set requestData->size to the size
 *						the desklet will be upon instantiating - the field will still contain the
 *						size of the desklet if the desklet doesn't change the size.
 * @return TRUE iff implemented.
 *
 * @remark this function can be called from a different thread that the thread
 *			that created the lpEnumData pointer.
 */
BOOL CALLBACK OnGetSkinPreviewBitmap(ENUMERATOR_DATA* lpEnumData, const SkinInfo* skin,
				PreviewRequestData* requestData);

/**
 * Called when the preview bitmap generation running in OnGetSkinPreviewBitmap must be stopped.
 *
 * @param lpEnumData - the pointer returned by OnCreateSkinEnumeratorW() for which information about a skin
 *						is requested.
 * @param	requestData - the requestData that was passed to OnGetSkinPreviewBitmap(). Changes made
 *							to this structure in OnGetSkinPreviewBitmap() will also be visible
 *							in the call to this function [it will be the same pointer].
 * 
 * @see OnGetSkinPreviewBitmap
 *
 * @return	TRUE iff implemented. The function must make sure OnGetSkinPreviewBitmap() has finished
 *				if this function returns.
 * @remark this function call be called from a different thread that the one that called 
 *			OnGetSkinPreviewBitmap()
 */
BOOL CALLBACK OnStopSkinPreviewBitmap(ENUMERATOR_DATA* enumerator, PreviewRequestData* requestData);
