...
{
public:
	CUnzipper(const char* szFileName = NULL);
	virtual ~CUnzipper();
	
	// simple interface
	BOOL Unzip(const char* szFileName, const char* szFolder = NULL, BOOL bIgnoreFilePath = FALSE);
	
	// works with prior opened zip
	BOOL Unzip(BOOL bIgnoreFilePath = FALSE); // unzips to output folder or sub folder with zip name 
	BOOL UnzipTo(const char* szFolder, BOOL bIgnoreFilePath = FALSE); // unzips to specified folder

	// extended interface
	BOOL OpenZip(const char* szFileName);
	BOOL CloseZip(); // for multiple reuse
	BOOL SetOutputFolder(const char* szFolder); // will try to create
	
	// unzip by file index
	int GetFileCount();
	BOOL GetFileInfo(int nFile, UZ_FileInfo& info);
	BOOL UnzipFile(int nFile, const char* szFolder = NULL, BOOL bIgnoreFilePath = FALSE);
	
	// unzip current file
	BOOL GotoFirstFile(const char* szExt = NULL);
	BOOL GotoNextFile(const char* szExt = NULL);
	BOOL GetFileInfo(UZ_FileInfo& info);
	BOOL UnzipFile(const char* szFolder = NULL, BOOL bIgnoreFilePath = FALSE);
...
protected:
	...
	BOOL CreateFilePath(const char* szFilePath); // truncates from the last '\'
	BOOL SetFileModTime(const char* szFilePath, DWORD dwDosDate);
