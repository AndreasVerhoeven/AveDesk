...
void CIconWindow::MakeIcon()
{
	...
	std::wstring alternateTip(_Policies.GetStringValue(L"runtime//trayIcon//tooltip", L"AveDesk 2.0"));
	if(alternateTip.length() > 0)
		wcscpy(_ncd.szTip, alternateTip.c_str());
	else
		wcscpy(_ncd.szTip,L"AveDesk 1.4");
...
LRESULT CIconWindow::OnInitMenuPopup(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
		if(!_Policies.GetBoolValue(L"runtime//trayIcon//themesMenuItem//allowCustomBrowsing", TRUE))
		{
...
{
	if(!_Policies.GetBoolValue(L"runtime//trayIcon//menu//allow", TRUE))
		return;
	...
	if(!_Policies.GetBoolValue(L"runtime//trayIcon//menu//themesMenuItem//allow", TRUE))
	...
	if(!_Policies.GetBoolValue(L"runtime//trayIcon//menu//showAddNewDeskletItem", TRUE))
		RemoveMenu(menu, ID_ADDADESKLET, MF_BYCOMMAND);

	if(!_Policies.GetBoolValue(L"runtime//trayIcon//menu//showShowCaseItem", TRUE))
		RemoveMenu(menu, !showCaseActive ? ID_OPENSHOWCASE : ID_CLOSESHOWCASE,MF_BYCOMMAND);

	if(!_Policies.GetBoolValue(L"runtime//trayIcon//menu//showCloseItem", TRUE))
