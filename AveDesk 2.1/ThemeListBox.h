#if !defined(AFX_THEMELISTBOX_H__20011106_AF04_B15C_D1F1_0080AD509054__INCLUDED_)
#define AFX_THEMEELISTBOX_H__20011106_AF04_B15C_D1F1_0080AD509054__INCLUDED_
...
class ATL_NO_VTABLE CThemeListBoxImpl : 
   public CWindowImpl< T, TBase, TWinTraits >,
   public COwnerDraw< CThemeListBoxImpl<T> >
{
public:
   DECLARE_WND_SUPERCLASS(NULL, TBase::GetWndClassName())

   CThemeListBoxImpl() : m_hSubFont(NULL)
   { 
   ...
   BEGIN_MSG_MAP(CThemeListBoxImpl)
      MESSAGE_HANDLER(WM_CREATE, OnCreate)
      MESSAGE_HANDLER(WM_DESTROY, OnDestroy)
      MESSAGE_HANDLER(WM_ERASEBKGND, OnEraseBackground)
      CHAIN_MSG_MAP_ALT(COwnerDraw<CThemeListBoxImpl>, 1)
   ...
   int AddItem(const ILBITEM* pItem)
   {
      ...
      // Add item to listbox
      int iItem = CListBox::AddString(pNewItem->pszText);
   ...
   void DrawItem(LPDRAWITEMSTRUCT lpdis)
   {
      ...
      UINT format = DT_LEFT | DT_NOPREFIX | DT_WORDBREAK;
      ...
      if( pItem->mask & ILBIF_SUBTEXT ) {
         ...
         RECT rcText = { 0 };
         dc.DrawText(pItem->pszSubText, -1, &rcText, format | DT_CALCRECT | DT_END_ELLIPSIS	);
         dc.DrawText(pItem->pszSubText, -1, &rcItem, format | DT_END_ELLIPSIS	);
         rcItem.top += rcText.bottom-rcText.top;
      }
...
class CThemeListBoxCtrl : public CThemeListBoxImpl<CThemeListBoxCtrl>
{
public:
   DECLARE_WND_SUPERCLASS(_T("WTL_ThemeListBox"), GetWndClassName())  
};
