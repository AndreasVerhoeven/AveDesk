...
bool StartApp()
{
	...
	// make a tray icon
	if(_Policies.GetBoolValue(L"loading//showTrayIcon", TRUE))
		...
		!_noLoadThemeOnStartUp)
	{
		...
		std::wstring polTheme(_Policies.GetStringValue(L"loading//themeFileName", theme));
	...
	if(_Policies.GetBoolValue(L"loading//doFirstBootIntro", TRUE) && _globalSettings.GetFirst13Boot())
	{
		...
		wcscat(intro, L"desklets\\AveDesk 1.3 Intro\\intro.dll");
...
BOOL UnpackResources(const std::wstring& path)
{
	...
	return unzipper.UnzipTo(W2A(path.c_str()),FALSE);	
}
