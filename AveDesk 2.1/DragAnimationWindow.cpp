// DragAnimationWindow.cpp: implementation of the CDragAnimationWindow class.
//
//////////////////////////////////////////////////////////////////////
...
CDragAnimationWindow::CDragAnimationWindow() :
	currentFrame(0),
	currentLargeFrame(0),
	gdipBmp(NULL),
	size(0),
...
{
	RECT r = {0};
	Create(NULL,r,_T("AveDesk Animation"),WS_POPUP | WS_OVERLAPPED,WS_EX_TOOLWINDOW | WS_EX_LAYERED | WS_EX_TOPMOST);
...
void CDragAnimationWindow::SafeDestroy()
{
	if(::IsWindow(m_hWnd))
	{
		bDestroyed = TRUE;
...
void CDragAnimationWindow::OnFinalMessage( HWND ) 
{ 
...
LRESULT CDragAnimationWindow::OnClose(UINT uMsg,WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if(bDestroyed)
		bHandled = FALSE;
...
BOOL CDragAnimationWindow::IsInsideForbiddenWindowArea()
{
...
LRESULT CDragAnimationWindow::OnMouseMove(UINT uMsg,WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	POINT pt = { GET_X_LPARAM(GetMessagePos()), GET_Y_LPARAM(GetMessagePos())};
	//GetCursorPos(&pt);
	pt.x -= wndSize.cx /2;
...
UINT CDragAnimationWindow::CheckAnimSpeed()
{
	UINT interval = 10;
...
void CDragAnimationWindow::UpdateLarge(SIZE s, SIZE s2, BYTE largeAlpha, BYTE smallAlpha, BOOL largeFirst)
{
	CDC dc;
	CBitmap bmp;
...
void CDragAnimationWindow::Update(int size, int alpha)
{
	CDC dc;
	CBitmap bmp;
	CWindowDC screenDC(NULL);
...
LRESULT CDragAnimationWindow::OnTimer(UINT uMsg,WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	else if(3 == wParam || 4 == wParam)
	{
		if(NULL == gdipBmpLarge)
		{
			...
			SIZE bmpS = {0};
			HBITMAP hBmp = NULL;
			if(entry->GetLargePreview(&hBmp, &bmpS))
			{
				smallAndLargeAreTheSame = entry->AreSmallAndLargePreviewTheSame();
		...
		if(NULL == gdipBmpLarge)
		{
			if(entry->FailedLoadingLargePreview())
			{
				KillTimer(3);
				KillTimer(4);
		...
		else
		{
			...
			SIZE s2 = s;//{fSize2, fSize2};
			wndSize = s;
			//sizeLarge = static_cast<int>(fSize);
...
LRESULT CDragAnimationWindow::OnCaptureChanged(UINT uMsg,WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	SafeDestroy();
	return 0;
}

LRESULT CDragAnimationWindow::OnKeyDown(UINT uMsg,WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CDragAnimationWindow::OnLButtonUp(UINT uMsg,WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	POINT pt = { GET_X_LPARAM(GetMessagePos()), GET_Y_LPARAM(GetMessagePos())};
	//POINT pt = {0};
	//GetCursorPos(&pt);
...
void CDragAnimationWindow::StartAnimation(CFoundDeskletEntry* e, const SIZE& s, HWND parentHwnd)
{
	...
	GetBitmapBits(e->GetPreviewBitmap(),data.Stride * data.Height, data.Scan0);
