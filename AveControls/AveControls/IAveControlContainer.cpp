...
BOOL IAveControlContainer::Message(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		IAveControl* ctrl = iter->second;
		if(ctrl != NULL && !ctrl->GetIsDisabled())
		{
...
void IAveControlContainer::SetFocusToControl(const char* layerName)
{
...
BOOL IAveControlContainer::MouseRightClick(UINT id, const char* layerName, const POINT* pt, const SIZE* s, DWORD keys)
{
	std::map<std::string, IAveControl*>::iterator iter = controls.find(layerName);
	if(iter != controls.end())
	{
		...
		if(ctrl != NULL)
		{
			if(!ctrl->GetIsDisabled())
			{
				SetFocusToControl(layerName);
				return ctrl->MouseRightClick(pt, s, keys);
			}
			else
			{
				MessageBeep(MB_ICONASTERISK);
