...
DWORD __stdcall Docklet::ThisThreadFunc(LPVOID lpData)
{
	...
	*/
	...
	if(Docklets.size() == 0)
	{
		if(_Policies.GetBoolValue(L"ui//desklets//exitWhenLastDeskletIsClosed", FALSE))
			_App->Close();
	}
	LeaveCriticalSection(&cs);
	//DeleteCriticalSection(&cs);
...
{
	if(!_Policies.GetBoolValue(L"ui//desklet//propertyDialog//allow", TRUE))
		return 0;
	...
	if(_Policies.GetBoolValue(L"ui//desklet//propertyDialog//appearance//allow", TRUE))
		propertyPages.push_back(CreatePropertySheetPage(appearanceSheet));

	CDeskletLabelSheet labelSheet(this);
	if(!SupportsFeature(DESKLET_NO_LABEL) && _Policies.GetBoolValue(L"ui//desklet//propertyDialog//label//allow", TRUE))
		propertyPages.push_back(CreatePropertySheetPage(labelSheet));

	CDeskletAdvancedSheet advancedSheet(this);
	if(_Policies.GetBoolValue(L"ui//desklet//propertyDialog//advanced//allow", TRUE))
		propertyPages.push_back(CreatePropertySheetPage(advancedSheet));

	if(_Policies.GetBoolValue(L"ui//desklets//propertyDialogs//other//allow", TRUE))
	{
...
HMENU Docklet::GetDefaultMenu(int startId)
{
	...
	BOOL noneAllowed = !_Policies.GetBoolValue(L"ui//desklet//menu//allow", TRUE);
	if(noneAllowed || !_Policies.GetBoolValue(L"ui//desklet//menu//showLockPositionItem", TRUE))
		RemoveMenu(menu, ID_DESKLETPOPUPMENU_LOCKPOSITION, MF_BYCOMMAND);

	if(noneAllowed || SupportsFeature(DESKLET_NO_LABEL) || !_Policies.GetBoolValue(L"ui//desklet//menu//showSubLabelItem", TRUE))
		RemoveMenu(menu, ID_DESKLETPOPUPMENU_SHOWSUBLABEL, MF_BYCOMMAND);

	if(noneAllowed || !_Policies.GetBoolValue(L"ui//desklet//menu//showDuplicateItem", TRUE))
		RemoveMenu(menu, ID_DESKLETPOPUPMENU_DUPLICATETHISDESKLET, MF_BYCOMMAND);

	if(noneAllowed || !_Policies.GetBoolValue(L"ui//desklet//menu//showCloseItem", TRUE))
		RemoveMenu(menu, ID_DESKLETPOPUPMENU_REMOVEDESKLET, MF_BYCOMMAND);

	if(noneAllowed || !_Policies.GetBoolValue(L"ui//desklet//menu//showPropertiesItem", TRUE))
		RemoveMenu(menu, ID_DESKLETPOPUPMENU_DESKLETPROPERTIES, MF_BYCOMMAND);

	if(noneAllowed || !_Policies.GetBoolValue(L"ui//desklet//menu//showControlPanelItem", TRUE))
