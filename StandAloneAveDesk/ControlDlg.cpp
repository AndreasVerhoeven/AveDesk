...
LRESULT CControlDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if(_Policies.GetBoolValue(L"ui//controlPanel//acceptDrags", TRUE))
	...
	if(!_Policies.GetBoolValue(L"ui//controlPanel//menu//allow", TRUE))
		SetMenu(NULL);
		

	std::wstring customTitle = _Policies.GetStringValue(L"ui//controlPanel//title", L"");
	if(customTitle.length() > 0)
		SetWindowTextW(customTitle.c_str());

	std::wstring customCopyRight = _Policies.GetStringValue(L"ui//controlPanel//copyright", L"");
	if(customCopyRight.length() > 0)
		SetDlgItemText(IDC_COPYRIGHT, customCopyRight.c_str());

	std::wstring customLinkText = _Policies.GetStringValue(L"ui//controlPanel//linkText", L"");
	...
	std::wstring customLink = _Policies.GetStringValue(L"ui//controlPanel//linkURL", L"");
...
LRESULT CControlDlg::OnContextMenu(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if(_Policies.GetBoolValue(L"ui//controlPanel//list//allowRightClickMenu", TRUE))
	{
		GetCursorPos(&p);
...
void CControlDlg::AddDeskletToList(Docklet* d)
{
	...
	if(_Policies.GetBoolValue(L"ui//controlPanel//list//showPreviewImage", TRUE))
		item.mask |= ILBIF_IMAGE;	

	if(_Policies.GetBoolValue(L"ui//controlPanel//list//showTitle", TRUE))
		item.mask |= ILBIF_TEXT;	

	if(_Policies.GetBoolValue(L"ui//controlPanel//list//showSubTitle", TRUE))
