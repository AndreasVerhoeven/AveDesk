...
Bitmap* __stdcall HBitmapToCppGDIPlusBitmap(HBITMAP hBitmap)
{
	...
	SIZE s = {0};
	if ( GetBitmapDimensionEx(hBitmap,&s) )
	{
...
void InitializeBitmapInfoHeader(HBITMAP bitmap,BITMAPINFOHEADER* BI, long Colors)
{
	...
	else if(Colors >= 257 && Colors <= 65536)
	{
		BI->biBitCount = 16;
		BI->biClrUsed = Colors;	
	}
	else if(Colors >= 65537 && Colors <= 16777216)
	{
		BI->biBitCount = 24;
		//BI->biClrUsed = Colors;		
	}
...
DWORD WriteIcon(HANDLE f, HBITMAP hColor, HBITMAP hMask, long& Length, long lColors)
{
	ICONINFO iconInfo = {0};
	...
	InternalGetDIBSizes(hColor,ColorInfoSize,ColorBitsSize,lColors);
	...
	InternalGetDIB(hColor,NULL,(BITMAPINFO*)ColorInfo,ColorBits,lColors);
...
BOOL __stdcall WriteIconEntry(ICONFILE* pIconFile, CREATEICONENTRY* pInfo)
{
	...
	/*int mod = dwNewFilePointer % 4;
	if(mod != 0)
	{
		DWORD dwWritten(0);
		unsigned long pad = 0L;
		WriteFile(f,&pad,mod,&dwWritten,NULL);
		dwNewFilePointer += mod;
	}*/
	...
	long length(0);
	WriteIcon((HANDLE)pIconFile->reserved1,pInfo->hColor,pInfo->hMask,length, pInfo->lColors);
