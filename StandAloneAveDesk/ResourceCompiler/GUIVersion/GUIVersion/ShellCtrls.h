...
#define TVN_CHECKEDSTATUS_CHANGED WM_APP + 102
...
public:
   ...
   BOOL _FilterItem(IShellFolder* pFolder, LPCITEMIDLIST pidl, DWORD& dwAttribs) const
   {
      ...
      dwAttribs |= SFGAO_DISPLAYATTRMASK | SFGAO_FOLDER | SFGAO_FILESYSTEM | SFGAO_FILESYSANCESTOR | SFGAO_BROWSABLE;
...
#define TREE_ITEM_GRAYED	2
#define TREE_ITEM_CHECKED	1
#define TREE_ITEM_UNCHECKED 0
...
public:
   ...
   END_MSG_MAP()
	...
	BOOL SetCheckState(HTREEITEM hItem, INT state)
	{
		TVITEM tvItem;
   ...
   BOOL _Init()
   {
      ...
      SetImageList(images.Detach(), TVSIL_NORMAL);
	  	...
	  	CImageList imgList = GetImageList(TVSIL_STATE);
		SIZE iconSize = {0};
		imgList.GetIconSize(iconSize);
		CDC dc;
		dc.CreateCompatibleDC(CWindowDC(m_hWnd));
		CBitmap bmp, mask;
		bmp.CreateCompatibleBitmap(CWindowDC(m_hWnd), iconSize.cx, iconSize.cy);
		mask.CreateCompatibleBitmap(CWindowDC(m_hWnd), iconSize.cx, iconSize.cy);
		dc.SelectBitmap(bmp);
		RECT rc = {1,1,iconSize.cx, iconSize.cy};
		CBrush brush;
		brush.CreateSolidBrush(RGB(255,255,255));
		CTheme theme;
		if(CTheme::IsThemingSupported() && theme.OpenThemeData(m_hWnd, L"BUTTON"))
		{
			theme.DrawThemeBackground(dc,	BP_CHECKBOX,CBS_MIXEDNORMAL, &rc, 0);
			theme.CloseThemeData();
		}
		else
		{
			dc.FillRect(&rc, brush);
			dc.DrawFrameControl(&rc, DFC_BUTTON,DFCS_BUTTONCHECK | DFCS_INACTIVE | DFCS_FLAT | DFCS_CHECKED);
		}
		dc.SelectBitmap(NULL);
		dc.SelectBitmap(mask);
		dc.FillRect(&rc, brush);
		dc.SelectBitmap(NULL);
		imgList.Add(bmp, mask);
                         ...
                         DWORD dwAttribs, HTREEITEM hParentItem)
   {
	  ...
	  BOOL isParentChecked = hParentItem != NULL 
							&& 	hParentItem != TVI_ROOT 
							&& (GetCheckState(hParentItem) == TREE_ITEM_CHECKED);
      ...
      item.item.cChildren = (dwAttribs & SFGAO_FILESYSANCESTOR);
      ...
      HTREEITEM newItem =  InsertItem(&item);
	  if(newItem != NULL && hParentItem != NULL)
	  {
		 if(isParentChecked)
			  SetCheckState(newItem, TREE_ITEM_CHECKED);
	  }
   ...
   void CheckRecursive(HTREEITEM hItem, INT newState)
   {
	   if(NULL == hItem)
		   return;

	   if(hItem != NULL && hItem != TVI_ROOT)
		   SetCheckState(hItem, newState);

	   HTREEITEM child = GetChildItem(hItem);
	   while(child != NULL)
	   {
		   SetCheckState(child, newState);
		   CheckRecursive(child, newState);
   ...
   LRESULT OnLeftButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
	  ...
	  TV_HITTESTINFO hitTest = {pt,0};
	  TreeView_HitTest(m_hWnd, &hitTest);
	  if(hitTest.flags & TVHT_ONITEMSTATEICON)
	  {
			INT state = GetCheckState(hitTest.hItem);
			if(TREE_ITEM_CHECKED == state)
			{
				SetCheckState(hitTest.hItem, TREE_ITEM_UNCHECKED);
				CheckRecursive(hitTest.hItem, TREE_ITEM_UNCHECKED);
				HTREEITEM hItem = GetParentItem(hitTest.hItem);
				while(hItem != NULL && hItem != TVI_ROOT)
				{
					state = GetCheckState(hItem);
					if(TREE_ITEM_CHECKED == state)
						SetCheckState(hItem, TREE_ITEM_GRAYED);
					hItem = GetParentItem(hItem);
				}
			}
			else
			{
				SetCheckState(hitTest.hItem, TREE_ITEM_CHECKED);
				CheckRecursive(hitTest.hItem, TREE_ITEM_CHECKED);
			...
			WPARAM wp = (WPARAM) GetDlgCtrlID();
			NMHDR hdr = {0};
			hdr.code = TVN_CHECKEDSTATUS_CHANGED;
			hdr.hwndFrom = m_hWnd;
			hdr.idFrom = GetDlgCtrlID();
			LPARAM lp = (LPARAM)&hdr;
   ...
   void PopulateChildNode(HTREEITEM hItem)
   {
	   TVITEM tvItem = {0};
	   ...
	   tvItem.hItem = hItem;
		...
		PSHELLITEMINFO pFolderItem = reinterpret_cast<PSHELLITEMINFO>(tvItem.lParam);
		CComPtr<IShellFolder> spFolder;
		if( pFolderItem->pidlNode != NULL ) {
		  if( FAILED(pFolderItem->spFolder->BindToObject(pFolderItem->pidlNode, NULL, IID_IShellFolder, (LPVOID*)&spFolder)) )return;
      }
      else {
      ...
      if( SUCCEEDED(spFolder->EnumObjects(NULL, dwEnumFlags, &spEnum)) ) {
         ...
         while( (spEnum->Next(1, &pidl, &dwFetched) == S_OK) && (dwFetched > 0) ) {
            ...
            if( !_FilterItem(spFolder, pidl, dwAttribs) ) {
               _InsertItem(spFolder, pFolderItem->pidlFull, pidl, dwAttribs, hItem);
			   noChildren = FALSE;
            }
            pidl.Delete();
         }
      ...
      DWORD dwEnumFlagsFiles = SHCONTF_NONFOLDERS;
      if( m_dwShellStyle & SCT_EX_SHOWHIDDEN ) dwEnumFlagsFiles |= SHCONTF_INCLUDEHIDDEN;
      if( SUCCEEDED(spFolder->EnumObjects(NULL, dwEnumFlagsFiles, &spEnumFiles)) ) {
         CPidl pidl;
         DWORD  dwFetched;
         while( (spEnumFiles->Next(1, &pidl, &dwFetched) == S_OK) && (dwFetched > 0) ) {
            // Get attributes and filter some items
            DWORD dwAttribs = SFGAO_DISPLAYATTRMASK | SFGAO_HASSUBFOLDER;
            if( !_FilterItem(spFolder, pidl, dwAttribs) ) {
               _InsertItem(spFolder, pFolderItem->pidlFull, pidl, dwAttribs, hItem);
	  ...
	  if(noChildren)
	  {
		TVITEM item = {TVIF_CHILDREN	| TVIF_HANDLE};
		item.cChildren = 0;
		item.hItem = hItem;
		SetItem(&item);
	  }
   ...
   LRESULT OnItemExpanding(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
   {
	  ...
	  //SetCheckState(pnmtv->itemNew.hItem, TREE_ITEM_GRAYED);

//#ifdef __ATLCTRLX_H__
      CWaitCursor cursor;
//#endif
		PopulateChildNode(pnmtv->itemNew.hItem);
