...
LPITEMIDLIST CPidl::Append(LPITEMIDLIST pidlBase, LPITEMIDLIST pidlAdd)
{
	CComPtr<IMalloc> malloc;
	...
	pBase.SetFromPIDL(pidlBase);
	pAdd.SetFromPIDL(pidlAdd);

	if(!pBase.IsValid() || !pAdd.IsValid())
	{
		pBase.Detach();
		pAdd.Detach();
		return NULL;
	...
	UINT cb1 = pBase.GetItemIDSize();
	UINT cb2 = pAdd.GetItemIDSize();

	pidlNew = (LPITEMIDLIST) malloc->Alloc(cb1 + cb2 + sizeof(USHORT));

	if (pidlNew != NULL)
	{
		memcpy(static_cast<void*>(pidlNew),static_cast<void*>(pidlBase),cb1);
		BYTE* b = (reinterpret_cast<BYTE*>(pidlNew) + cb1) ;
		memcpy(static_cast<void*>(b),pidlAdd,cb2);
		*(reinterpret_cast<USHORT*>(b + cb2) ) = 0;			
	}
...
template<typename T> T HexStringTo(const std::wstring& str)
{
	T val = (T)wcstol(str.c_str(),NULL,16);
...
CPidl::operator LPITEMIDLIST() const
{
	return m_Pidl;
}

CPidl::operator bool()const
{
	return IsValid();
}

bool CPidl::IsValid()const
{
	return m_Pidl != NULL;
}

bool CPidl::operator == (const LPITEMIDLIST& pidl)const
{
...
bool CPidl::operator == (const CPidl& that)const
{
	return operator==(that.m_Pidl);
}

std::wstring CPidl::SHITEMIDToString(SHITEMID* itemID)
{
	if (itemID == NULL)
		return L"";

	std::wstringstream ws;
	ws << std::hex << std::setw(sizeof(USHORT)*2) << std::setfill(L'0') << itemID->cb;
	for(int i=0;i<itemID->cb;++i)
	{
		ws << std::hex << std::setw(sizeof(BYTE)*2) << std::setfill(L'0') << (int)itemID->abID[i];
	}

	return ws.str();
}

std::wstring CPidl::StringToSHITEMID(std::wstring s, SHITEMID* itemID)
{
	if(itemID == NULL)
		return s;

	USHORT cb = HexStringTo<USHORT>(s.substr(0,sizeof(USHORT)*2) );
	s.erase(0,sizeof(USHORT)*2);
	...
	for(int i=0;i<cb;++i)
	{
		itemID->abID[i] = (BYTE)HexStringTo<USHORT>(s.substr(0,sizeof(BYTE)*2) );
		s.erase(0,sizeof(BYTE)*2);
	}

	return s;
}

std::wstring CPidl::ToString()
{
	std::wstringstream ws;
	ws << std::hex;

	USHORT cb = (USHORT)GetItemIDSize();

	BYTE* bytePointer = reinterpret_cast<BYTE*>(m_Pidl);
	for(int pos =0; pos < cb;++pos)
	{
		ws << std::setw(sizeof(BYTE)*2) << std::setfill(L'0') << (int) (*bytePointer);
		bytePointer++;
	}
...
HRESULT CPidl::FromString(const std::wstring& str)
{
	ClearCache();
	FreePidl(m_Pidl);
	...
	UINT cb = str.length()/2;
	pidl = static_cast<LPITEMIDLIST>(malloc->Alloc(cb+sizeof(USHORT))); 

	if( pidl == NULL )
		return E_FAIL;

	BYTE* bytePointer = reinterpret_cast<BYTE*>(pidl);
	for(UINT i=0;i<cb*2;i+=2)
	{
		(*bytePointer) = HexStringTo<BYTE>(str.substr(i,2));
		bytePointer++;
	}
...
HRESULT CPidl::GetUIObject(HWND hWnd, REFIID riid, void** p)
{
	if(m_Pidl == NULL)
		return E_FAIL;
	...
	return parentFolder->GetUIObjectOf(hWnd,1,const_cast<LPCITEMIDLIST*>(&relativePidl),riid,NULL,p);

}

HRESULT CPidl::GetIContextMenu(IContextMenu** menu, HWND hWnd)
{
	return GetUIObject(hWnd,IID_IContextMenu,reinterpret_cast<void**>(menu));
}
...
UINT CPidl::TrackItemIDContextMenu(HMENU hMenu, UINT nFlags, const POINT& point, HWND hWnd, CMINVOKECOMMANDINFOEX& cmi)
{
	...
	boost::shared_ptr<AVECMSUBCLASSDATA>scData (new AVECMSUBCLASSDATA);
...
HICON CPidl::ExtractIcon(UINT size, UINT flags)
{
	...
	_hRes = extractIcon->GetIconLocation(GIL_FORSHELL,location,MAX_PATH,&index,&outFlags);
