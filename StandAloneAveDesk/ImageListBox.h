...
#define ILBS_VCENTERTEXT		 0x0100
...
class ATL_NO_VTABLE CImageListBoxImpl : 
   ...
   public COwnerDraw< CImageListBoxImpl<T> >
{
public:
   DECLARE_WND_SUPERCLASS(NULL, TBase::GetWndClassName())

   CImageListBoxImpl() : m_hSubFont(NULL), useDoubleBuffering(TRUE)
   { 
   ...
   BEGIN_MSG_MAP(CImageListBoxImpl)
      ...
      CHAIN_MSG_MAP_ALT(COwnerDraw<CImageListBoxImpl>, 1)
   ...
   void DrawItem(LPDRAWITEMSTRUCT lpdis)
   {
      ...
      UINT format = DT_LEFT | DT_NOPREFIX | DT_WORDBREAK | DT_END_ELLIPSIS | DT_EDITCONTROL;
      ...
      if( pItem->mask & ILBIF_TEXT ) {
         ...
         RECT rcText = { 0 };
         dc.DrawText(pItem->pszText, -1, &rcText, format | DT_CALCRECT | DT_EDITCONTROL);
         ...
         dc.DrawText(pItem->pszText, -1, &rcItem, format | DT_EDITCONTROL);
      ...
      if( pItem->mask & ILBIF_SUBTEXT ) {
         ...
         RECT rcText = { 0 };
         dc.DrawText(pItem->pszSubText, -1, &rcText, format | DT_CALCRECT | DT_WORDBREAK | DT_END_ELLIPSIS | DT_EDITCONTROL	);
         dc.DrawText(pItem->pszSubText, -1, &rcItem, format | DT_WORDBREAK | DT_END_ELLIPSIS | DT_EDITCONTROL);
         rcItem.top += rcText.bottom-rcText.top;
      }
