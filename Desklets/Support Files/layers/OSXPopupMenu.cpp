...
LRESULT OnMenuCreate(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(data->pt.x + bmp->GetWidth() > desktopRect.right)
		data->pt.x = (desktopRect.right -  bmp->GetWidth());
	if(data->pt.y + bmp->GetHeight() > desktopRect.bottom)
...
LRESULT OnMenuMouseMove(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(data != NULL)
	{
		...
		if(xPos >= 0 && xPos <= data->menuWidth + checkMarkWidth  && yPos >= 0 && yPos <= itemHeight * data->data.count)
		{
