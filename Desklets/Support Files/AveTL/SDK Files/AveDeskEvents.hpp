...
template<class T> BOOL __stdcall OnDragOver(T* t,IDataObject* obj,DWORD keys ,POINTL pnt ,DWORD* effect)
{
	return t->DragOver(obj,keys,pnt,effect);
}
#define EVENT_ONDRAGOVER(t) template BOOL __stdcall OnDragOver(t*,IDataObject*,DWORD,POINTL ,DWORD*);
...
#define EVENT_ONPROCESSMESSAGE(t) template void __stdcall OnProcessMessage<t>(t*,hwnd,msg,WPARAM,LPARAM);


template<class T> int __stdcall OnAddPropertySheets(T* t, int numPresent)
{
	return t->OnAddPropertySheets(numPresent);
}
...
{
	return t->DragDropDataRequest(t,pFetc,pMedium);
}
#define EVENT_ONDRAGDROPDATAREQUEST(t) BOOL __stdcall OnDragDropDataRequest<t>(t*,FORMATETC*, STGMEDIUM*);
