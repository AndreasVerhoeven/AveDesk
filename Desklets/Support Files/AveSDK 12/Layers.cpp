...
BOOL CLayers::Exists(std::string const& name)const
{
	BOOL success(FALSE);
	DeskletLayerGetLayer(m_ownerHWnd,name.c_str(),&success);
...
CLayer& CLayers::operator[](std::string const& name)
{
	std::map<std::string,CLayer>::iterator iter = m_layers.find(name);
	if(iter == m_layers.end() )
	{
		...
		{
			layerId = ::DeskletLayerAdd(m_ownerHWnd, name.c_str());
			m_layers.insert( std::make_pair(name,CLayer(this,name)) );
		}
...
void CLayers::RemoveAll()
{
	while(Size() > 0)
	{
		::DeskletLayerRemove(m_ownerHWnd,0,FALSE);
	}
}
