...
// possible controls (30 Ocotober 2005)
...
// A font structure used by controls which text is editable (textfield, editbox).
struct AVE_FONT
{
	WCHAR faceName[LF_FACESIZE];	// the name of the face of the font
	UINT size;						// the size of the font, in integer points.
	UINT style;						// A binary or (|) combination of the GDI+ styles for the font.
};

// A font structure used by controls which text is NOT editable (label, dropdownlist, etc).
// this font structure does not use the same size factors as the AVE_FONT structure.
struct AVE_FONT2
{
	WCHAR faceName[LF_FACESIZE];	// the name of the face of the font
	FLOAT size;						// the size of the font, in floatingpoint points.
	UINT style;						// A binary or (|) combination of the GDI+ styles for the font.
};
...
// A structure used to desribe a dropshadow
struct AVE_SHADOWPROPERTIES
{
	DWORD dwSize;		// sizeof(AVE_SHADOWPROPERTIES)

	INT xOffset;		// the x-offset of the shadow in pixels
	INT yOffset;		// the y-offset of the shadow in pixels
	INT extraWidth;		// the number of pixels the shadow is stretched horizontally
	INT extraHeight;	// the number of pixels the shadow is stretched vertically
	ALPHA_RGB color;	// the color of the value in the ARGB format.
	FLOAT blurRatio;	// the ratio that describes how much the shadow is blurred.
	UINT intensity;		// the intensity of the shadow. An intensity of 10 is normal.
};
