...
{
	strcpy(szName, "Shortcut Desklet");
	strcpy(szAuthor, "Andreas Verhoeven");
	(*iVersion) = 110;
	strcpy(szNotes,"A very simple shortcut desklet.");
}
...
BOOL CALLBACK OnDrop(DOCKLET_DATA* lpData,IDataObject* obj,DWORD keys ,POINTL pnt ,DWORD* effect)
{
	if(IsDataObjectHDROP(obj))
	{
		...
		else
		{
			...
			char* pFrom = new char[num*MAX_PATH+4];
			pFrom[0] = '"';
			char* pFromCur = pFrom+1;
			...
			*pFromCur = 0;
			...
			ShellExecute(0,"open",lpData->path,pFrom,lpData->path,openMode);
	...
	if(uMsg == WM_TIMER && wParam == IMAGE_TIMER)
	{
		if(strcmp(lpData->path,"")!=0)
		{
			...
			if(hFile)
			{
				...
				char postfix[100];
				strcpy(postfix," bytes");

				if(size  >= 1024){size/=1024;strcpy(postfix," kB");}
				if(size  >= 1024){size/=1024;strcpy(postfix," MB");}
				if(size  >= 1024){size/=1024;strcpy(postfix," GB");}

				itoa(size,fs,10);
				strcat(fs,postfix);

				DeskletSetSubLabel(lpData->GetWindow(),fs);
			}
