	...
	isInRightClickMenu(FALSE)
{
	...
	editHwnd = CreateWindow(TEXT("EDIT"), "", flags, 0, 0, font->GetHeight(&g), 20000, GetContainer()->GetDeskletHandle(), NULL, HINST_THISCOMPONENT, NULL);
}
	...
	switch(msg)
	{
		...
		case AV_SET_FONT:
			...
			else
			{
				...
				SetFont(new Font(af->faceName, af->size, af->style));
...
void CAveEditBox::Update()
{
	...
	RectF layoutRect(-GetScrollX(), 0, AVE_EDITBOXMAXWIDTH, r.bottom - r.top);

	std::wstring text = GetDrawingString();

	BOOL createdNewBmp = FALSE;
	if(NULL == bmp || bmp->GetWidth() != visibleRect.Width || bmp->GetHeight() != visibleRect.Height)
	{
		bmp = new Bitmap(visibleRect.Width, visibleRect.Height);
		createdNewBmp = TRUE;
	}
