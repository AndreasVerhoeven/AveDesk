...
UINT GetStringLength(const WCHAR* str, UINT itemHeight)
{
	...
	path.AddString(str, -1, &fontFamily, AVE_OSX_MENU_STYLES, (float)AVE_OSX_MENU_FONTSIZE, rect, &format);
...
BOOL LoadMenuResource(Bitmap** bitmap, const char* resourcesPath, const char* file)
{
	...
	*bitmap = NULL;
	...
	strcpy_s(path, MAX_PATH, resourcesPath);
...
BOOL CopyMenuData(InternalMenuData* internal, const MenuData* data)
{
	...
	for(UINT i = 0; i < internal->data .count; ++i)
	{
		if(data->items[i] != NULL)
			internal->data .items[i] = _wcsdup(data->items[i]);
		else
			internal->data.items = NULL;
	}
...
LRESULT OnMenuDestroy(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	InternalMenuData* data = reinterpret_cast<InternalMenuData*>(RemoveProp(hwnd, AVE_OSX_INTERNAL_MENU_DATA_PROP_NAME));
	if(data != NULL)
	{
