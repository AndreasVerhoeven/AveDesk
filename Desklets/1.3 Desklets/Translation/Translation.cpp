...
#pragma comment(lib, "../../../SDK/AveSDK/AveControls.lib")
#include "../../../SDK/AveSDK/aveconstants.h"
#include "../../../SDK/AveSDK/aveflags.h"
#include "../../../SDK/AveSDK/avecontrols.h"
...
public:
	...
	void SetupBackgroundLayers()
	{
		...
		DeskletLayerSetToolTipText(hwnd, flipLayer, L"Click to flip the desklet to its backside.");
		...
		DeskletLayerSetPlacement(hwnd, logoLayer, FALSE, TRUE, &skin.backLogo, skin.back.cx, skin.back.cy);
		DeskletLayerSetIsClickThrough(hwnd, logoLayer, FALSE);
		SetImageFileToLayer(logoLayer, L"logo.png");
		DeskletLayerSetToolTipText(hwnd, logoLayer, L"Powered by AveDesk.");
	}
	...
	void SetupControls()
	{
		...
		UINT lid = DeskletLayerGetLayer(hwnd, "back_done", NULL);
		DeskletLayerSetToolTipText(hwnd, lid, L"Click to flip this desklet to the frontside again.");
	/*	AveLockControl(doneBut);
		AveSendMessage(doneBut, AV_HIDE, 0L, 0L);
		AveSendMessage(doneBut,AV_SET_NORMALIMAGEFILE, FALSE, (LPARAM)L"done.png");
		AveSendMessage(doneBut,AV_SET_DOWNIMAGEFILE, FALSE, (LPARAM)L"done_pressed.png");
		...
		*/
		...
		lid = DeskletLayerGetLayer(hwnd, "textA", NULL);
		DeskletLayerSetToolTipText(hwnd, lid, L"Enter here the text you wish to translate,\nHit the enter key to translate the text.");
		...
		lid = DeskletLayerGetLayer(hwnd, "textB", NULL);
		DeskletLayerSetToolTipText(hwnd, lid, L"The above text again, but translated.");
		/*AveLockControl(textFieldB);
		AveSendMessage(textFieldB, AV_SET_ONENTERCALLBACK, (WPARAM)this, (LPARAM)OnEnterPressed);
		AveSendMessage(textFieldB, AV_SET_FONT, 0L, (LPARAM)&skin.textfieldToFont);
		AveSendMessage(textFieldB, AV_SET_TEXTCOLOR, 0L, (LPARAM)skin.textfieldToColor);
		...
		lid = DeskletLayerGetLayer(hwnd, "static_from", NULL);
		DeskletLayerSetToolTipText(hwnd, lid, L"Pick the language in which is the text you wish to translate.");
		/*AveLockControl(staticFrom);
		AveSendMessage(staticFrom, AV_SET_FONT2, 0L, (LPARAM)&skin.comboboxFromFont);
		AveSendMessage(staticFrom, AV_SET_TEXTCOLOR, 0L, (LPARAM)skin.comboboxFromColor);
		AveSendMessage(staticFrom, AV_SET_SHADOWPROPERTIES, skin.comboboxFromDrawShadow, (LPARAM)&skin.comboboxFromShadow);
		...
		lid = DeskletLayerGetLayer(hwnd, "static_to", NULL);
		DeskletLayerSetToolTipText(hwnd, lid, L"Pick the language you wish to translate the text to.");
		/*AveLockControl(staticTo);
		AveSendMessage(staticTo, AV_SET_FONT2, 0L, (LPARAM)&skin.comboboxToFont);
		AveSendMessage(staticTo, AV_SET_TEXTCOLOR, FALSE, (LPARAM)skin.comboboxToColor);
		AveSendMessage(staticTo, AV_SET_SHADOWPROPERTIES, skin.comboboxToDrawShadow, (LPARAM)&skin.comboboxToShadow);
...
{
	strcpy_s(szName,100,"Translator");
	strcpy_s(szAuthor,100,"Andreas Verhoeven");
	strcpy_s(szNotes,100,"Translates");
	*iVersion = 130;
}
...
BOOL __stdcall OnLayerMouseDown(Translator* t, UINT layerId, const char* layerName, const POINT* pt, const SIZE* s, DWORD keys)
{
	...
	else if(strcmp(layerName, "static_from") == 0 || strcmp(layerName, "popup_top") == 0)
	{
		...
		strcpy_s(resourcesPath, MAX_PATH, WStringToString(t->CreateSkinnedPath(L"controls\\menu\\")).c_str());
		...
		DoOSXPopupMenu(t->hwnd, WM_MENU_LANG_FROM, &data, resourcesPath, &pt);
	...
	else if(strcmp(layerName, "static_to") == 0 || strcmp(layerName, "popup_bottom") == 0)
	{
		...
		DoOSXPopupMenu(t->hwnd, WM_MENU_LANG_TO, &data, resourcesPath, &pt);
...
BOOL __stdcall OnGetFlags(Translator* t, DWORD* flags)
{

	// when AveDesk 2.x queries for features that this desklet doesn't want
	// we can indicate which features we don't want: for example, we can turn of
	// labels.

	if(flags != NULL)
		*flags = DESKLET_NO_LABEL;
