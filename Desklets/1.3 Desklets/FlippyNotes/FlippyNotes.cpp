...
public:
	...
	{
		AVE_FONT f = {0};
		wcscpy(f.faceName, GetSelFontName().c_str());
	...
	void SetupControls()
	{
		...
		AVE_FONT2 f2 = {0};
		wcscpy(f2.faceName, L"Lucida Sans Unicode");
...
void __stdcall OnGetInformation(char *szName, char *szAuthor, int *iVersion, char *szNotes)
{
	...
	strcpy(szName,"StickyNotes");
	strcpy(szAuthor,"Andreas Verhoeven");
	strcpy(szNotes,"Add little sticky notes on your desktop.");
	*iVersion = 130;
}
