...
void CFlipAnimation::RebindTexture(bool isFront, Bitmap* bmp)
{
	if(isFront)
	{
		frontBitmap = bmp;
		SafeRelease(frontTexture);
		InitTexture(&frontTexture, frontBitmap);
	}
	else
	{
		backBitmap = bmp;
		SafeRelease(backTexture);
		InitTexture(&backTexture, backBitmap);
	}
}
...
HRESULT CFlipAnimation::SetUpVertices()
{
	...
	vertices[0].u = 0.0f;
	vertices[0].v = 0.0f;
	vertices[0].n= D3DXVECTOR3(0,0,-1);
	...
	vertices[1].u = texW;
	vertices[1].v = 0.0f;
	vertices[1].n= D3DXVECTOR3(0,0,-1);
	...
	vertices[2].u = texW;
	vertices[2].v = texH;
	vertices[2].n= D3DXVECTOR3(0,0,-1);
	...
	vertices[3].u = 0.0f;
	vertices[3].v = texH;
	vertices[3].n= D3DXVECTOR3(0,0,-1);
...
void CFlipAnimation::SwapBackAndFront()
{
	std::swap(frontBitmap, backBitmap);
...
HRESULT CFlipAnimation::Render2D()
{
	...
	float camDistance = LookupCamDistance();
	D3DXVECTOR3 eyeVec(0.0f, 0.0f, camDistance);
	hRes = DoLightning(eyeVec);

	// if there is no rotation, we use a different filtering
	// for better detail.
	if(!rotationIsZero)
	{
		directDevice->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_ANISOTROPIC); //D3DTEXF_ANISOTROPIC
		directDevice->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_ANISOTROPIC);  //D3DTEXF_ANISOTROPIC
