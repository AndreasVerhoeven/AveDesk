...
void AveDrawString(Graphics* graphics, Brush* brush, const WCHAR* str, FontFamily* fontFamily, FontStyle fs, REAL fontSize, Rect& rc, StringFormat* strFormat, bool drawShadow, CShadowProperties& shadowProperties)
{
	...
	if(drawShadow)
	{
		...
		RectF rDraw(5,5,rcBounds.Width,rcBounds.Height);
		g2.DrawImage(&tmp,rDraw,rcBounds.X,rcBounds.Y,rcBounds.Width,rcBounds.Height,UnitPixel,NULL);

		std::auto_ptr<Bitmap> blurred ( GetBlur(&shadow,blurRatio) );
		if(blurred.get() )
		{
