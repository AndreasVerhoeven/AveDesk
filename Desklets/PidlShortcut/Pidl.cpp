...
{
	T val = wcstol(str.c_str(),NULL,16);
...
std::wstring CPidl::StringToSHITEMID(std::wstring s, SHITEMID* itemID)
{
	...
	for(int i=0;i<cb;++i)
	{
		itemID->abID[i] = HexStringTo<USHORT>(s.substr(0,sizeof(BYTE)*2) );
