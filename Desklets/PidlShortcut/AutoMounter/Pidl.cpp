...
std::wstring SHITEMIDToString(SHITEMID* itemID)
{
	if (itemID == NULL)
		return L"";
...
std::wstring StringToSHITEMID(std::wstring s, SHITEMID* itemID)
{
	if(itemID == NULL)
		return s;
...
std::wstring CPidl::ToString()
{
	...
	{
		ret += SHITEMIDToString(&pidl->mkid);
		pidl = reinterpret_cast<LPITEMIDLIST>( (reinterpret_cast<BYTE*>(pidl)) + pidl->mkid.cb );
	}

	ws.str(L"");
	ws << std::hex << std::setw(sizeof(USHORT)*2) << std::setfill(L'0') << 0;
	ret += ws.str();
	*/
